<template>
  <div class="audio-box">
    ...
    <div class="music-control">
      ...
      <div class="control-item" @click="goComment">
        <img src="../../../assets/img/player/comment_white.svg" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    musicTime() {
      return timer => {
        return (
          Math.floor(timer / 59).toFixed(0) +
          ":" +
          "00".concat((timer % 59).toFixed(0)).slice(-2)
        );
      };
    }
  },
  ...
  methods: {
    ...
    timeBarClick(event) {
      ...
      this.$refs.audio.currentTime =
        ((event.x - event.view.innerWidth * 0.1) /
          (event.view.innerWidth * 0.8)) *
        this.allTime;
    },
    changeMode() {
    ...
    play() {
      ...
      this.playInterval = setInterval(() => {
        this.allTime = isNaN(this.audio.duration)
          ? 0
          : this.audio.duration.toFixed(0);
        this.currentTime = this.audio.currentTime.toFixed(0);
        // 可能时间条还没挂在好，需要判断
        if (this.$refs.timeNow) {
          this.$refs.timeNow.style.right =
            100 - (this.currentTime * 100) / this.allTime + "%";
        }
