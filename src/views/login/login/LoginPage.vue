              ...
              <div class="card-body">
                ...
                <form method="POST" class="my-login-validation">
                  <div class="form-group">
                    <label :for="type[current_index]">{{
                      admin[current_index]
                    }}</label>
                    <input
                      :id="type[current_index]"
                  ...
                  <div class="mt-4 text-center">
                    没有账号?
                    <router-link to="/getcaptcha?type=register"
                      >点我创建</router-link
                    >
                  </div>
                </form>
  ...
  methods: {
    ...
    login() {
      if (this.current_index === 0) {
        // 手机登录
        phoneLogIn(this.account, this.password)
          .then(res => {
            if (res.data && res.data.code === 502) {
              ...
              (res && res.toString().indexOf("400") !== -1) ||
              (res && res.toString().indexOf("501") !== -1)
            ) {
        ...
        // 邮箱登录
        emailLogIn(this.account, this.password)
          .then(res => {
